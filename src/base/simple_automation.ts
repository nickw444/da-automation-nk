...
export class SimpleAutomationHelper {
    public readonly subDevice: ReturnType<TServiceParams["synapse"]["device"]["register"]>

    // private automationEnabledSwitch: ReturnType<TServiceParams["synapse"]["switch"]>
    private automationEnabledSwitch: { onUpdate: (cb: (newState: { state: string|number }, oldState: { state: string|number }) => void) => void };
    ...
    constructor(
        ...
        this.subDevice = synapse.device.register("da_automation_" + toSnakeCase(name), {
